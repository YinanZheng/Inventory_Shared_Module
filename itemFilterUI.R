itemFilterUI <- function(
  ...
  card_style <- sprintf("padding: 10px; border: 1px solid %s; border-radius: 8px; box-shadow: 0px 4px 6px rgba(0,0,0,0.1);", border_color)
  ...
  div(
    ...
    div(
      ...
      actionButton(ns("reset_btn"), "重置筛选", icon = icon("rotate-right"), class = "btn-danger",
    ...
    fluidRow(
      column(12, div(
        ...
        actionButton(ns("clear_other"), label = "", icon = icon("xmark", style = "color: #D32F2F;"),
                     style = "padding: 0 5px; border: none; margin-bottom: 14px; font-size: 18px; background-color: #F5F5F5; height: 45px; min-width: 34px;")
      ))
    ),
    
    # 日期范围筛选（使用 airDatepickerInput）
    tagList(
      if (use_purchase_date) {
        fluidRow(column(12, airDatepickerInput(ns("purchase_date_range"), "采购日期范围",
                                               range = TRUE, value = c(default_start_date, default_end_date), 
                                               autoClose = TRUE, width = input_width)))
      },
      if (use_exit_date) {
        div(
          airDatepickerInput(ns("exit_date_range"), "出库日期范围",
                             range = TRUE, value = c(default_start_date, default_end_date), 
                             autoClose = TRUE, width = input_width),
      ...
      if (use_sold_date) {
        div(
          airDatepickerInput(ns("sold_date_range"), "售出日期范围",
                             range = TRUE, value = c(default_start_date, default_end_date), 
                             autoClose = TRUE, width = input_width),
